...
//        $code = ($store->getCode()=='default') ? '' : $store->getCode();
        if (is_null($this->_filePath)) {
            $this->_filePath = $this->_deleteSpareSlashes(Mage::getBaseDir() . '/' . $this->getSitemapPath());
        }
    ...
    public function generateXml($entity=false) {
        ...
        if (!$entity || $entity=='tag') {
            ...
            if ($productTags) {
                ...
                foreach ($tags as $item) {
                    $tagUrl = $item->getTaggedProductsUrl();
                    $tagUrl = str_replace(Mage::getUrl(), '', $tagUrl);
                    $tagUrl = $this->_trailingSlash($baseUrl . $tagUrl);
                    $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>'."\n",
        ...
        if ((!$entity && (string)Mage::getConfig()->getModuleConfig('AW_Blog')->active=='true') || $entity=='blog') {
            ...
            if (!empty($collection) && count($collection)) {
                foreach ($collection as $item) {
                    $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>',
                            htmlspecialchars($this->_trailingSlash($baseUrl . $route . '/' . $item->getIdentifier())),
                            $date,
                            $changefreq,
                            $priority
                         );
