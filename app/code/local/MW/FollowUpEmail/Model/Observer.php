...
mage::log($cart['customer_email']);
			if($cart['customer_email'] == "") continue;
			
			if(in_array($cart['customer_email'],$customerEmails)){
				$rules = Mage::getModel('followupemail/rules')->getCollection()->loadRulesByEvent(MW_FollowUpEmail_Model_System_Config_Eventfollowupemail::EVENT_TYPE_ABANDON_CART,$storeId,$groupId)->getData();
				
			}			
			else{
				$rules = Mage::getModel('followupemail/rules')->getCollection()->loadRulesByEvent(MW_FollowUpEmail_Model_System_Config_Eventfollowupemail::EVENT_TYPE_ABANDON_CART_FOR_GUEST,$storeId,$groupId)->getData();	
