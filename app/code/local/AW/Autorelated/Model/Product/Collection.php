...
class AW_Autorelated_Model_Product_Collection extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Collection
{
    ...
    public function joinCategoriesByProduct()
    {
        $conditions = array(
            ...
            $this->getConnection()->quoteInto('cat_index.store_id = ?', Mage::app()->getStore()->getId())
        );
        ...
        $categoryIds = Mage::getModel('catalog/category')->load(Mage::app()->getStore()->getRootCategoryId())->getAllChildren();
        $this->getSelect()->join(array('root_category' => $this->getTable('catalog/category_product')), "cat_index.product_id = root_category.product_id AND cat_index.category_id = root_category.category_id AND root_category.category_id IN({$categoryIds})", array());
