...
class FME_Quickrfq_Block_Adminhtml_Quickrfq_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
     ...
     /* get the status of this rfq */
      $getstatus_query="SELECT status FROM quickrfq where quickrfq_id=$id";
	...
	/* display the status of this rfq */
     ...
     $fieldset->addField('create_date', 'label', array(
          'label'     => Mage::helper('quickrfq')->__('Date Quote requested: '),
     ...
     $fieldset->addField('update_date', 'label', array(
          'label'     => Mage::helper('quickrfq')->__('Date Quote updated Previously: '),
     ...
     /* display the company name of this rfq */
      $fieldset->addField('company', 'text', array(
          'label'     => Mage::helper('quickrfq')->__('Company Name: '),
      ...
      $fieldset->addField('contact_name', 'text', array(
          'label'     => Mage::helper('quickrfq')->__('Contact Name: '),
	...
	$getemail_query="SELECT email FROM quickrfq where quickrfq_id=$id";
	...
	/* display the date quote needed by the customer */
	 ...
	 /* display the project title of this rfq */
        $fieldset->addField('project_title', 'text', array(
          'label'     => Mage::helper('quickrfq')->__('Project Title: '),
 ...
 $getupload_query="SELECT prd FROM quickrfq where quickrfq_id=$id";
 ...
 if (!empty($file_name)) 
 {
  ...
  /* display the budget of this rfq */
        $fieldset->addField('budget', 'select', array(
          'label'     => Mage::helper('quickrfq')->__('Budget Status: '),
          'name'      => 'budget',
          'values'    => array(
              array(
                  'value'     => 'Approved',
                  'label'     => Mage::helper('quickrfq')->__('Approved'),
              ),

              array(
                  'value'     => 'Approval Pending',
                  'label'     => Mage::helper('quickrfq')->__('Approval Pending'),
              ),
	      
	       array(
                  'value'     => 'Open',
                  'label'     => Mage::helper('quickrfq')->__('Open'),
              ),
	       
	        array(
                  'value'     => 'No Approval',
                  'label'     => Mage::helper('quickrfq')->__('No Approval'),
              ),
          ),
      ));
